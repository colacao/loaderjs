...
(function(globle) {
		 ...
		 */
		var _get = function(src, config, index) {
				function c() {
				...
				config = config || {}, d.charset = config.charset || "utf-8", d.onload = d.onreadystatechange = function() {
						(/loaded|complete/i.test(this.readyState) || -1 == navigator.userAgent.toLowerCase().indexOf("msie")) && (config.onLoad && config.onLoad.call(this, index), c())
				}, d.onerror = function() {
		...
		var _initCss = function() {
				...
				if (exports.debug) {
						for (var i = 0; i < exports.css.list.length; i++) {
								...
								_checkUpdate(ls, ver[ver.length - 1], true, "s");

								_modes.push([ls, ver[ver.length - 1], ext[ext.length - 1].split('.')[ext[ext.length - 1].split('.').length - 1] + ".js", path.slice(0, path.length - 1).join('/')]);

								var _urls = _getUrls("dest/", _modes);
								_get.call(this, _urls, {
										onLoad: function(index) {
												var ret = _execute("s");
												_emit("onLoadCSSInit", arguments);
													if (index == exports.css.list.length - 1) {
														exports.css.state = 1;
		...
		var _initJS = function() {
				...
				if (exports.debug) {
						for (var i = 0; i < exports.js.list.length; i++) {
								...
								_checkUpdate(ls, ver[ver.length - 1], true, "m");

								_modes.push([ls, ver[ver.length - 1], ext[ext.length - 1].split('.')[ext[ext.length - 1].split('.').length - 1], path.slice(0, path.length - 1).join('/')]);
								var _urls = _getUrls("dest/", _modes);
								_get.call(this, _urls, {
										onLoad: function(index) {
												var ret = _execute("m");
												_emit("onLoadInitJS", arguments);
												if (index == exports.js.list.length - 1) {
														exports.js.state = 1;
